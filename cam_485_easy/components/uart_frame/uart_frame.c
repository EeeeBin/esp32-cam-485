...
frame_state_n frame_state;
QueueHandle_t uart_buffer_quenue;
#define RX_BUF_SIZE 1024*200
...
void uart_init(int baudrate) {
    ...
    uart_set_pin(UART_NUM_1, 4, 13, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
...
void uart_frame_task(void *arg) {
    ...
    for(;;) {
        uart_get_buffered_data_len(UART_NUM_1, &buf_len);
        switch(frame_state) {
            case IDLE: 
            ...
            case FINISH:
                ...
                uart_read_bytes(UART_NUM_1, rx_buffer->buf, rx_buffer->buf_size, 1);
